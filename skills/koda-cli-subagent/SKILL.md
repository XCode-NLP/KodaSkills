...
description: "Навык делегирования подзадач субагенту koda-cli. Позволяет быстрее решить комплекснык задачи из множества шагов благодаря параллельной работе с субагентом."
license: MIT
metadata:
  author: koda
  version: "1.0.0"
...
1. Предварительный анализ (декомпозиция) задачи указывает на возможность распараллеливания (например, правка тестов в одном модуле и документации в другом).
...
1. Если команды `koda` нет, установи её: `npm install --global @kodadev/koda-cli@latest`.
2. Если команды `npm` нет или пользователь отказался от установки:
