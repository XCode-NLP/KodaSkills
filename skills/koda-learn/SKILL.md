---
name: koda-learn
description: Позволяет искать и устанавливать дополнительные навыки (скиллы), расширяющие функционал агента, чтобы агент научился выполнять специальные задачи. Помогает подобрать подходящие навыки для текущего проекта. Навыки нужны для фокусирования агента на способах решения конкретных задач и обогащения контекста дополнительными знаниями. Активируется инструментом `read_skill` (vscode/jetbrains) или `Activate Skill` (cli) с передачей аргумента `koda-learn`.
license: MIT
metadata:
  author: koda
  version: 1.0.0
...
1. Пользователь может согласиться установить все найденные, некоторые из них или ни одного.
2. Если результаты поиска не подходят, остановись, предложив пользователю найти навык самостоятельно и установить в директорию установки.
...
1. Установка навыка допустима только в случае, если выполняются обязательные требования.
2. Приоритет у того навыка, который суммарно удовлетворяет большему количеству требований по всем критериям.
...
#### Источник 3 - другие каталоги навыков
...
2. Ошибки файловой системы при недостатке прав на запись.

**Действия:**

1. Если один из источников (вариантов установки) не подходит, нужн использовать второй.
2. Если использованы оба источника, но успех не достигнут, следует:
   - сообщить пользователю о возникшей ошибке и возможных вариантах её исправления;
   - предложить пользователю найти навык самостоятельно в этих двух истлочниках.
